...
int main() {
  ...
  while (q--) {
    ...
    if (t == 1) {
      cin >> l >> r;
      pair<node *, node *> res1 = split(T, r + 1);
      pair<node *, node *> res2 = split(res1.first, l);
      push(res2.second);
      cout << res2.second->S << "\n";
      T = merge(merge(res2.first, res2.second), res1.second);
    }

    if (t == 2) {
      cin >> x >> pos;
      pair<node *, node *> res = split(T, pos);
      T = merge(merge(res.first, new node(x)), res.second);
    }

    if (t == 3) {
      cin >> pos;
      pair<node *, node *> res1 = split(T, pos + 1);
      pair<node *, node *> res2 = split(res1.first, pos);
      T = merge(res2.first, res1.second);
    }
